...
class TestAutoRemediation:
    ...
    def test_should_auto_remediate_enabled(
    ...
    def test_should_auto_remediate_disabled(self, app_context, db_session, sample_node):
        ...
        stream = Stream(
            node_id=sample_node.id, path="test/disabled", auto_remediate=False
        )
    ...
    def test_should_auto_remediate_cooldown(
        self, app_context, db_session, sample_stream
    ):
    ...
    def test_should_auto_remediate_after_cooldown(
        self, app_context, db_session, sample_stream
    ):
    ...
    def test_should_auto_remediate_circuit_breaker(
        self, app_context, db_session, sample_stream
    ):
    ...
    def test_determine_start_level_no_history(
        self, app_context, db_session, sample_stream
    ):
    ...
    def test_determine_start_level_many_failures(
        self, app_context, db_session, sample_stream
    ):
    ...
    def test_try_restart_sidecar_get_config_failure(
        self, app_context, db_session, sample_stream, mock_httpx
    ):
    ...
    def test_try_restart_path_success(
        self, app_context, db_session, sample_stream, mock_httpx
    ):
    ...
    def test_try_restart_path_no_source(self, app_context, db_session, sample_node):
        ...
        stream = Stream(node_id=sample_node.id, path="test/no-source", source_url=None)
        db_session.add(stream)
        db_session.commit()
    ...
    def test_try_restart_mediamtx_success(
        self, app_context, db_session, sample_stream, mock_subprocess
    ):
        """Test successful MediaMTX restart."""
        mock_subprocess["run"].return_value = MagicMock(
            returncode=0, stdout="mediamtx", stderr=""
        )
    ...
    def test_try_restart_mediamtx_failure(
        self, app_context, db_session, sample_stream, mock_subprocess
    ):
    ...
    def test_try_restart_mediamtx_timeout(
        self, app_context, db_session, sample_stream, mock_subprocess
    ):
    ...
    def test_remediate_stream_success_first_try(
    ...
    def test_remediate_stream_creates_events(
        self, app_context, db_session, sample_unhealthy_stream, mock_httpx
    ):
    ...
    def test_remediate_stream_force_level(
        self, app_context, db_session, sample_unhealthy_stream, mock_httpx
    ):
        ...
        with patch("time.sleep"):
            ...
            result = remediation.remediate_stream(
                sample_unhealthy_stream, force_level=2
            )
    ...
    def test_remediate_stream_all_levels_fail(
        self,
        app_context,
        db_session,
