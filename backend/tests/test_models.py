...
class TestMediaMTXNode:
    ...
    def test_node_streams_relationship(self, app_context, db_session, sample_node):
        ...
        stream = Stream(node_id=sample_node.id, path="test/stream1", name="Test Stream")
...
class TestStream:
    ...
    def test_stream_recordings_relationship(
...
class TestStreamEvent:
    ...
    def test_event_default_severity(self, app_context, db_session, sample_stream):
        ...
        event = StreamEvent(
            stream_id=sample_stream.id, event_type=EventType.FPS_DROP.value
        )
        db_session.add(event)
        db_session.commit()
    ...
    def test_event_resolved(self, app_context, db_session, sample_stream):
        ...
        event = StreamEvent(
            stream_id=sample_stream.id, event_type=EventType.DISCONNECTED.value
        )
        db_session.add(event)
        db_session.commit()
...
class TestRecording:
    ...
    def test_recording_default_segment_type(
        self, app_context, db_session, sample_stream
    ):
    ...
    def test_recording_event_triggered(
        self, app_context, db_session, sample_stream, sample_event
    ):
...
"""
    ...
    def test_snapshot_rollback_reference(self, app_context, db_session, sample_node):
        ...
        original = ConfigSnapshot(
            node_id=sample_node.id, config_hash="original", config_yaml="paths: {}"
        )
