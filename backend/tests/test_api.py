...
class TestHealthAPI:
    ...
    def test_get_health_status(self, app_context, client):
        ...
        with patch("app.api.health.checker.check_redis", return_value="ok"), patch(
            "app.api.health.checker.check_mediamtx_api", return_value="ok"
        ):
    ...
    def test_get_streams_health_filtered(
        self, app_context, client, sample_node, sample_stream
    ):
    ...
    def test_probe_stream(
        self, app_context, client, sample_stream, mock_subprocess, ffprobe_response
    ):
    ...
    def test_probe_url(self, app_context, client, mock_subprocess, ffprobe_response):
        ...
        response = client.post(
            "/api/health/probe",
            json={"url": "rtsp://localhost:8554/test", "protocol": "rtsp"},
        )
    ...
    def test_quick_check_all(
        self, app_context, client, sample_node, mock_httpx, mediamtx_api_response
    ):
...
class TestStreamsAPI:
    ...
    def test_create_stream_invalid_node(self, app_context, client):
        ...
        response = client.post(
            "/api/streams/", json={"node_id": 99999, "path": "test/stream"}
        )
    ...
    def test_update_stream(self, app_context, client, sample_stream):
        ...
        response = client.put(
            f"/api/streams/{sample_stream.id}",
            json={"name": "Updated Name", "auto_remediate": False},
        )
    ...
    def test_get_thumbnail_not_found(self, app_context, client, sample_stream):
        ...
        with patch(
            "app.api.streams.thumbnail_service.get_cached_thumbnail", return_value=None
        ):
    ...
    def test_generate_thumbnails_batch(self, app_context, client, sample_stream):
        ...
        with patch("app.api.streams.thumbnail_service.generate_thumbnail"):
            response = client.post(
                "/api/streams/thumbnail/batch",
                json={"stream_ids": [sample_stream.id], "sync": True},
            )
...
class TestFleetAPI:
    ...
    def test_update_node(self, app_context, client, sample_node):
        ...
        response = client.put(
            f"/api/fleet/nodes/{sample_node.id}", json={"environment": "production"}
        )
...
class TestConfigAPI:
    ...
    def test_validate_config(self, app_context, client):
        ...
        response = client.post(
            "/api/config/validate", json={"config_yaml": "paths:\n  test: {}"}
        )
    ...
    def test_plan_config(self, app_context, client, sample_node, mock_httpx):
        ...
        response = client.post(
            "/api/config/plan",
            json={"node_id": sample_node.id, "config_yaml": "paths:\n  new: {}"},
        )
...
class TestRecordingsAPI:
    ...
    def test_get_retention_status(self, app_context, client):
        ...
        with patch(
            "app.api.recordings.retention_mgr.get_status",
            return_value={
                "disk": {
                    "total_gb": 500,
                    "used_gb": 200,
