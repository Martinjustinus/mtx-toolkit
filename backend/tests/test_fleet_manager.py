...
class TestFleetManager:
    ...
    def test_sync_node_streams_success(
        self, app_context, db_session, sample_node, mock_httpx, mediamtx_api_response
    ):
    ...
    def test_sync_node_streams_api_error(
    ...
    def test_sync_node_streams_connection_error(
        self, app_context, db_session, sample_node, mock_httpx
    ):
    ...
    def test_sync_node_streams_updates_existing(
        self, app_context, db_session, sample_node, sample_stream, mock_httpx
    ):
    ...
    def test_sync_node_streams_deletes_stale(
        self, app_context, db_session, sample_node, mock_httpx
    ):
        """Test that sync deletes stale streams."""
        # Create a stream that won't be in the API response
        stale_stream = Stream(
            node_id=sample_node.id, path="stale/stream", name="Stale Stream"
        )
    ...
    def test_sync_all_nodes_partial_failure(
        self, app_context, db_session, sample_node, mock_httpx
    ):
        """Test sync all nodes with some failures."""
        # Create second node
        node2 = MediaMTXNode(
            name="node-2", api_url="http://192.168.1.11:9997", is_active=True
        )
    ...
    def test_get_node_health_healthy(
        self, app_context, db_session, sample_node, mock_httpx
    ):
    ...
    def test_get_node_health_unhealthy(
        self, app_context, db_session, sample_node, mock_httpx
    ):
    ...
    def test_get_node_health_connection_error(
        self, app_context, db_session, sample_node, mock_httpx
    ):
    ...
    def test_check_all_nodes_health(
        self, app_context, db_session, sample_node, mock_httpx
    ):
    ...
    def test_apply_policy_to_fleet(self, app_context, db_session, sample_stream):
        ...
        result = manager.apply_policy_to_fleet(
            {"auto_remediation_enabled": False, "recording_enabled": True}
        )
    ...
    def test_apply_policy_to_fleet_by_environment(
        self, app_context, db_session, sample_node, sample_stream
    ):
        ...
        manager = FleetManager()
        result = manager.apply_policy_to_fleet(
            {"auto_remediation_enabled": False}, environment="production"
        )

        assert result["success"] is True
        assert result["streams_updated"] >= 1

    def test_apply_policy_to_fleet_no_match(
        self, app_context, db_session, sample_node, sample_stream
    ):
    ...
    def test_get_fleet_metrics(
        self,
        app_context,
        db_session,
        sample_node,
        sample_stream,
        sample_unhealthy_stream,
    ):
    ...
    def test_get_fleet_metrics_by_environment(
    ...
    def test_rolling_update_config_not_found(
        self, app_context, db_session, sample_node
    ):
    ...
    def test_rolling_update_no_nodes(
        self, app_context, db_session, sample_config_snapshot
    ):
        ...
        manager = FleetManager()
        result = manager.rolling_update(config_snapshot_id=sample_config_snapshot.id)

        assert result["success"] is False
        assert "No nodes found" in result["error"]

    def test_rolling_update_success(
        self, app_context, db_session, sample_node, sample_config_snapshot, mock_httpx
    ):
    ...
    def test_rolling_update_batch_failure(
        self, app_context, db_session, sample_node, sample_config_snapshot, mock_httpx
    ):
    ...
    def test_rolling_update_by_environment(
        self, app_context, db_session, sample_node, sample_config_snapshot, mock_httpx
    ):
        ...
        manager = FleetManager()
        result = manager.rolling_update(
            environment="production", config_snapshot_id=sample_config_snapshot.id
        )

        # Should only update production node
        assert result["total_nodes"] == 1

    def test_sync_updates_node_last_seen(
        self, app_context, db_session, sample_node, mock_httpx, mediamtx_api_response
    ):
