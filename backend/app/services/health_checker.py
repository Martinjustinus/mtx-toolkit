...
from app import db
from app.models import EventType, MediaMTXNode, Stream, StreamEvent, StreamStatus
...
class HealthChecker:
    ...
    def __init__(self):
        self.timeout = (
            current_app.config.get("HEALTH_CHECK_TIMEOUT", 10) if current_app else 10
        )
        ...
        try:
            ...
            for stream in streams:
                ...
                if old_status != stream.status:
                    self._create_status_change_event(
                        stream, old_status, {"quick_check": True}
                    )
    ...
    def _measure_fps(self, url: str, duration: float = 2.0) -> Optional[float]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=self.timeout + duration + 5
            )
    ...
    def _analyze_probe_result(
        self, probe_data: Dict, url: str, protocol: str
    ) -> Dict[str, Any]:
    ...
    def _create_status_change_event(
        self, stream: Stream, old_status: str, probe_result: Dict
    ):
        ...
        event = StreamEvent(
            stream_id=stream.id,
            event_type=(
                EventType.DISCONNECTED.value
                if stream.status == StreamStatus.UNHEALTHY.value
                else EventType.RECONNECTED.value
            ),
            severity=severity,
    ...
    def get_all_streams_health(
        self, node_id: Optional[int] = None, status: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                "total": len(streams),
                "healthy": sum(
                    1 for s in streams if s.status == StreamStatus.HEALTHY.value
                ),
                "degraded": sum(
                    1 for s in streams if s.status == StreamStatus.DEGRADED.value
                ),
                "unhealthy": sum(
                    1 for s in streams if s.status == StreamStatus.UNHEALTHY.value
                ),
                "unknown": sum(
                    1 for s in streams if s.status == StreamStatus.UNKNOWN.value
                ),
            },
        }
    ...
    def detect_black_screen(self, url: str, duration: float = 2.0) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=self.timeout + duration
            )
    ...
    def detect_freeze(self, url: str, duration: float = 5.0) -> bool:
        ...
        try:
            cmd = [
                ...
                str(duration),
                "-vf",
                f"freezedetect=n=0.003:d={self.FREEZE_DETECTION_DURATION}",
                "-an",
                "-f",
                "null",
                "-",
